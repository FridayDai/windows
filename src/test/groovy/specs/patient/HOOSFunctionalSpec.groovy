...
//    @Ignore
	...
	//    @Ignore
	def "check HOOS complete score successfully"() {
		when: "At HOOS CompletePage"
		at TaskCompletePage

		then: "Close window and back to gmail"
		waitFor(3, 1) {
			$(scoresItem[0]).text().trim() == "Symptoms Score: 50"
			$(scoresItem[1]).text().trim() == "Pain Score: 48"
			$(scoresItem[2]).text().trim() == "ADL Score: 51"
			$(scoresItem[3]).text().trim() == "Sport/Rec Score: 63"
			$(scoresItem[4]).text().trim() == "QOL Score: 38"
		}
	}
